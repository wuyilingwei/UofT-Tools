...
import { state, addExtraCourse, removeExtraCourse, isValidCourseCode } from '../store.js'

const props = defineProps({
  label: { type: String, default: 'Add a course (outside your programs)' },
  placeholder: { type: String, default: 'Search code or name, e.g. CSC263…' },
...
const list = computed(() => props.items ?? state.extraCourses)
...
<template>
  <div class="course-picker">
    ...
    <div v-if="list.length" class="cp-chips">
      <span v-for="code in list" :key="code" class="cp-chip">
...
<style scoped>
.course-picker { margin-top: 10px; position: relative; }
.cp-label {
